...
export default function ContactPage() {
  ...
  const [formData, setFormData] = useState(
    typeof window !== "undefined" &&
      (JSON.parse(localStorage.getItem("formData")) ?? initState)
  );

  function newToast(type, message) {
    setToasts((old) => [
      ...old,
      <Toast key={old.length} id={old.length} type={type} message={message} />,
    ]);
  }
  ...
  return (
    <>
      <div className="alerts grid gap-2 fixed top-0 left-0">{toasts}</div>
      ...
      <form
        onSubmit={(e) => (isWaiting ? e.preventDefault() : submitHandler(e))}
      ...
      >
        ...
        <button className="btn-primary w-full" disabled={isWaiting}>
          {isWaiting ? "Loading..." : "Submit"}
        </button>
        {isWaiting && <Loading />}
      </form>
    </>
