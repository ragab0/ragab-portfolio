...
export default function Toast({ type, message, id }) {
  ...
  const types = {
    success: {
      Ico: Success,
      currentStyle:
        "text-green-500 after:bg-green-500 border-green-500/10 bg-emerald-900",
    },
    failed: {
      Ico: Failed,
      currentStyle:
        "text-red-500 after:bg-red-500 border-red-500/10 bg-[#3e2d24]",
    },
    warning: {
      Ico: Warning,
      currentStyle:
        "text-yellow-600 after:bg-yellow-600 border-yellow-600/10 bg-[#3c382e]",
    },
  };
  ...
  return (
    <div
      ref={myToast}
      id={id}
      className={`toast relative top-4 left-4 mb-2 z-10 border-b-[6px] p-2 pl-4 min-w-[300px] inline-flex items-center 
