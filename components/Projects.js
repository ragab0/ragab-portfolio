      ...
      <section className="projects">
        {
          projectsApi.map(({name, description, skills, srcImg, link, gitLink, inProgress}, i) => (
            <figure key={i} className="flex my-16 gap-4 overflow-hidden rounded-md p-4 hover:bg-gray-100 
              dark:hover:bg-emerald-900/25 duration-300 ease-out"
            > 
              <Link href={link || "#"} target={link ? "_blank" : "_top"} 
                className="transition-none self-center overflow-hidden rounded-[inherit] h-[450px] 
                bg-red-400 text-current"
              >
                <Image alt={name.toLocaleLowerCase()} src={srcImg} className="min-w-full min-h-full object-cover 
                  hover:scale-110" 
                />
              </Link>
              <figcaption className="py-2 px-4 max-w-md ">
                ...
                <Link href={gitLink || "#"} target={gitLink ? "_blank" : "_top"}
